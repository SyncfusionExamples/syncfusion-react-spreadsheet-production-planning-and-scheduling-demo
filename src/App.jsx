...
import { demandData, materialData } from './dataSource';
...
export default function App() {
  ...
  const fields = { text: 'role' };
  const roleChangeHandler = (args) => {
    const selected = args.itemData;
    // map each role to the target sheet name
    const map = {
      Planner: 'Demand',
      Supervisor: 'Schedule',
      Manager: 'DashBoard'
    };
    const sheetName = map[selected.role] || 'DashBoard';
    const spreadsheetRef = spreadsheet;
    setTimeout(() => {
      // navigate to the sheet
      spreadsheetRef.goTo(`${sheetName}!A1`);
    }, 50);
    //applying sheet protection based on user role
    applyProtectionByRole(sheetName, selected.role);
  };

  const applyProtectionByRole = (activeSheetName, userRole) => {
    spreadsheet.sheets.forEach((sheets) => {
      if (sheets.name === activeSheetName) {
        spreadsheet.unprotectSheet(activeSheetName, { selectCells: true, selectUnLockedCells: true, formatCells: true, formatRows: true, formatColumns: true, insertLink: false });
      }
      else {
        spreadsheet.protectSheet(sheets.name, { selectCells: true, selectUnLockedCells: true, formatCells: true, formatRows: true, formatColumns: true, insertLink: false });
      }
    });
  ...
  const onCreated = () => {
    ...
    spreadsheet.merge('Dashboard!A1:B4');
    ...
    appendShortagesToSchedule();
    //sheet protection during the initial render
    spreadsheet.protectSheet(1);
    spreadsheet.protectSheet(2);
    spreadsheet.protectSheet(3);
    spreadsheet.resize();
  };

  //updating shortages to schedule sheet
  const appendShortagesToSchedule = () => {
    const demandSheet = getSheet(spreadsheet, 1);
    const materialSheet = getSheet(spreadsheet, 2);
    const scheduleSheet = getSheet(spreadsheet, 3);
    ...
    //Setting Empty Rows to updated Rows to avoid duplicates
  ...
  return (
    <>
      <div style={{ margin: '12px 0' }}>
        <label style={{ marginRight: 8 }}>Select User Role:</label>
      ...
      <SpreadsheetComponent ref={(ssObj) => { spreadsheet = ssObj; }} created={onCreated.bind(this)} height={750} cellSave={onCellSave} showFormulaBar={true} isProtected={true} showRibbon={true} >
        <SheetsDirective>
          ...
          <SheetDirective name='Demand' showGridLines={false}>
            <RangesDirective>
              <RangeDirective dataSource={demandData}> </RangeDirective>
            </RangesDirective>
            <ColumnsDirective>
          ...
          <SheetDirective name='Material' showGridLines={false}>
            <RangesDirective>
              <RangeDirective dataSource={materialData}> </RangeDirective>
            </RangesDirective>
            <ColumnsDirective>
              <ColumnDirective width={250}></ColumnDirective>
              <ColumnDirective width={150}></ColumnDirective>
